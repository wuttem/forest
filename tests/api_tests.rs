...
use forest::models::{AuthConfig, Tenant, TenantId};
...
async fn test_full_user_management_flow() {
    ...
    assert_eq!(
        res.status().as_u16(),
        200,
        "Expected 200 OK for tenant creation"
    );
    ...
    let res = client
        .post(&format!(
            "{}/test_tenant/devices/device_id_1/passwords",
            api_url
        ))
    ...
    assert_eq!(
        res.status().as_u16(),
        200,
        "Expected 200 OK for password creation"
    );
    ...
    assert_eq!(
        res.status().as_u16(),
        200,
        "Expected 200 OK for fetching passwords"
    );
