...
impl CertificateManager {
    ...
    fn save_private_key_absolute(
        ...
        let key_string = String::from_utf8(key_pem).map_err(|_| {
            CertificateError::ValidationError("Invalid UTF-8 in private key".to_string())
        })?;
    ...
    fn save_certificate_absolute(&self, cert: &X509, file_path: &Path) -> CertResult<String> {
        ...
        // Convert to string to return
        let key_string = String::from_utf8(cert_pem).map_err(|_| {
            CertificateError::ValidationError("Invalid UTF-8 in certificate".to_string())
        })?;
    ...
    fn load_private_key_absolute(&self, path: &Path) -> CertResult<PKey<Private>> {
        ...
        PKey::private_key_from_pem(&contents).map_err(|e| e.into())
    }
    ...
    fn load_certificate_absolute(&self, path: &Path) -> CertResult<X509> {
        ...
        X509::from_pem(&contents).map_err(|e| e.into())
    }
    ...
    pub fn get_ca_cert_pem(&self) -> CertResult<String> {
        ...
        if !ca_cert_path.exists() {
            return Err(CertificateError::FileNotFound(
    ...
    pub fn is_server_cert_valid(&self, server_name: &str, host_names: &[&str]) -> CertResult<bool> {
            ...
            Some(entry) => match entry.data().as_utf8() {
                ...
                Err(_) => {
                    return Err(CertificateError::InvalidCertificate(
                        "Common name is not valid UTF-8".to_string(),
                    ))
                }
            },
            None => {
                return Err(CertificateError::MissingData(
                    "Certificate is missing Common Name".to_string(),
                ))
            }
        };
        ...
        let required_hostnames: HashSet<String> =
            host_names.iter().map(|&s| s.to_string()).collect();
    ...
    pub fn create_server_cert_with_hostnames(
        &self,
        server_name: &str,
        host_names: &[&str],
    ) -> CertResult<()> {
